...
FLASHPrimaryGeneratorMessenger::FLASHPrimaryGeneratorMessenger(
                                             FLASHPrimaryGeneratorAction* FLASHGun)
:FLASHAction(FLASHGun)
{ 
  //
...
void FLASHPrimaryGeneratorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)                
{/*
  if ( command == meanKineticEnergyCmd )                                                                        
    { FLASHAction -> SetmeanKineticEnergy(meanKineticEnergyCmd
						  -> GetNewDoubleValue(newValue));}
  if ( command == sigmaEnergyCmd )                                                                        
    { FLASHAction -> SetsigmaEnergy(sigmaEnergyCmd
					    -> GetNewDoubleValue(newValue));}*/
  if ( command == XpositionCmd )                                                                        
    { FLASHAction -> SetXposition(XpositionCmd
					  -> GetNewDoubleValue(newValue));}

  if ( command == YpositionCmd )                                                                        
    { FLASHAction -> SetYposition(YpositionCmd
					  -> GetNewDoubleValue(newValue));}

  if ( command == ZpositionCmd )                                                                        
    { FLASHAction -> SetZposition(ZpositionCmd
					  -> GetNewDoubleValue(newValue));}

  if ( command == sigmaYCmd )                                                                        
    { FLASHAction -> SetsigmaY(sigmaYCmd
				       -> GetNewDoubleValue(newValue));}

  if ( command == sigmaZCmd )                                                                        
    { FLASHAction -> SetsigmaZ(sigmaZCmd
				       -> GetNewDoubleValue(newValue));}

if ( command == ThetaCmd )                                                                        
    { FLASHAction -> SetTheta(ThetaCmd
				       -> GetNewDoubleValue(newValue));}


/*
  if ( command == sigmaMomentumYCmd )                                                                        
    { FLASHAction -> SetsigmaMomentumY(sigmaMomentumYCmd
					       -> GetNewDoubleValue(newValue));}

  if ( command == sigmaMomentumZCmd )                                                                        
    { FLASHAction -> SetsigmaMomentumZ(sigmaMomentumZCmd
					       -> GetNewDoubleValue(newValue));}
*/
}                 
